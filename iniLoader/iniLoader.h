...
namespace cdq {
    ...
    duration ini_value::as_duration(size_t index) const {
        duration ret;
        clear_data(ret);
    ...
    IniFile::IniFile() { _current_node_ptr = &_root_node; }

    IniFile::~IniFile() {}

    int IniFile::load_stream(std::istream &in, bool is_append) {
    ...
    int IniFile::read(const char *file_path, bool is_append) {
        if (NULL == file_path) {
    ...
    int IniFile::read(const std::string &file_path, bool is_append) { return read(file_path.c_str(), is_append); }
    ...
    void IniFile::set_section(const std::string &path) { _current_node_ptr = &get_node(path, &get_root_node()); }

    ini_value &IniFile::get_section() { return *_current_node_ptr; }

    const ini_value &IniFile::get_section() const { return *_current_node_ptr; }


    ini_value &IniFile::get_root_node() { return _root_node; }

    const ini_value &IniFile::get_root_node() const { return _root_node; }

    ini_value &IniFile::get_node(const std::string &path, ini_value *father_ptr) {
        if (NULL == father_ptr) {
    ...
    ini_value &IniFile::get_child_node(const std::string &path, ini_value *father_ptr) {
        if (NULL == father_ptr) {
            father_ptr = &_root_node;
        }
    ...
    void IniFile::write(const std::string &path, bool &val, bool is_force, size_t index) {
    ...
    void IniFile::write(const std::string &path, std::string &val, bool is_force, size_t index) {
        ini_value &cur_node = get_node(path);
    ...
    void IniFile::write(const std::string &path, char *begin, char *end, bool is_force, size_t index) {
        ini_value &cur_node = get_node(path);
        if (cur_node.has_data() || is_force) {
